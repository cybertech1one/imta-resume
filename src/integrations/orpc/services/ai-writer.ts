	...
	generateBulletPoints: async (input: {
		...
		if (input.save) {
			...
			await db.insert(schema.aiWriterContent).values({
				...
				name: `Bullet Points - ${new Date().toLocaleDateString("fr-FR")}`,
	...
	generateSummary: async (input: {
		...
		if (input.save) {
			...
			await db.insert(schema.aiWriterContent).values({
				...
				name: `Resume - ${new Date().toLocaleDateString("fr-FR")}`,
	...
	quantifyAchievement: async (input: {
		...
		if (input.save) {
			...
			await db.insert(schema.aiWriterContent).values({
				...
				name: `Achievement - ${new Date().toLocaleDateString("fr-FR")}`,
	...
	extractSkills: async (input: {
		...
		if (input.save) {
			...
			await db.insert(schema.aiWriterContent).values({
				...
				name: `Skills - ${new Date().toLocaleDateString("fr-FR")}`,
	...
	generateCoverLetter: async (input: {
		...
		if (input.save) {
			...
			await db.insert(schema.aiWriterContent).values({
				...
				name: `Lettre - ${input.companyName || "Sans nom"} - ${new Date().toLocaleDateString("fr-FR")}`,
	...
	optimizeLinkedIn: async (input: {
		...
		if (input.save) {
			...
			await db.insert(schema.aiWriterContent).values({
				...
				name: `LinkedIn - ${new Date().toLocaleDateString("fr-FR")}`,
