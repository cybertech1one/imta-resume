...
const BASE_URL = "https://imta-resume-app-production.up.railway.app";
...
function record(endpoint, res, note = "") {
  ...
  const isEmpty = hasData && (
    (Array.isArray(res.data.json) && res.data.json.length === 0) ||
    res.data.json === null
  );
...
main()
  .then(({ results, ok, broken, skipped }) => {
    ...
    process.stdout.write(JSON.stringify({ results, summary: { total: results.length, ok: ok.length, broken: broken.length, skipped: skipped.length } }, null, 2));
    process.stdout.write("\n___JSON_END___\n");
    process.exit(0);
  })
  .catch((err) => {
